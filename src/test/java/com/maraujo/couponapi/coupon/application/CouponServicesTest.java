...
class CouponServicesTest {
    ...
    CreateCouponCommand command(String code) {
        return new CreateCouponCommand(code, "Original", new BigDecimal("0.5001"), NOW.plusSeconds(60), true);
    }

    Coupon coupon() {
        return Coupon.create(ID, "ABC123", "Original", BigDecimal.ONE, NOW, false, NOW.minusSeconds(60));
    }
    ...
    void invalidDomainNeverWrites() {
        ...
        assertThatThrownBy(() -> service.execute(command("ABC"))).isInstanceOf(BusinessRuleViolation.class);
    ...
    void failedCreationDoesNotReturnSuccess() {
        ...
        assertThatThrownBy(() -> new CreateCouponService(repository, CLOCK, () -> ID).execute(command("ABC123")))
    ...
    void deletedCouponIsNotFoundOnNormalRead() {
        when(repository.findById(ID)).thenReturn(Optional.of(new StoredCoupon(coupon().delete(NOW), 8)));
        assertThatThrownBy(() -> new GetCouponService(repository).execute(ID))
                .isInstanceOf(CouponNotFoundException.class);
    }
